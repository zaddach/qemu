...
#error MAX_SERIAL_PORTS must be at least 3!
    ...
    qdev_prop_set_int32(s->nand, "shift", 1);
    if (dmtd && blk_by_legacy_dinfo(dmtd)) {
        qdev_prop_set_drive_nofail(s->nand, "drive", blk_by_legacy_dinfo(dmtd));
    }
    qdev_init_nofail(s->nand);
    sysbus_connect_irq(SYS_BUS_DEVICE(s->nand), 0,
    ...
    if (dsd && blk_by_legacy_dinfo(dsd)) {
        omap3_mmc_attach(s->cpu->omap3_mmc[1], blk_by_legacy_dinfo(dsd), 0, 1);
    }
    if ((dsd = drive_get(IF_SD, 0, 1)) != NULL && blk_by_legacy_dinfo(dsd)) {
